...
async function calc_total_part(){

    let video_select = db.prepare('select wvid,max(part) as part from video group by wvid')
    let wvids = video_select.all()

    let part_update = db.prepare('update video set total_part = ? where wvid = ?')

    // console.log(wvids)
    for(let i of wvids){
        let wvid = i.wvid
        let part = i.part
        part_update.run(part,wvid)
        console.log(wvid,part)
    }
